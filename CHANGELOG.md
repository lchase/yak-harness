...
- Documentation site scaffold (#28): a private `docs-site/` Docusaurus
  3.10 workspace (independent `package.json`, so the root's `zod`/`ajv`
  runtime-dep rule is untouched), mirroring yak's `docs-site/` —
  `src/css/custom.css` copied verbatim, autogenerated sidebar, root
  redirect to the first doc. `.github/workflows/docs-deploy.yml` builds
  and deploys to GitHub Pages (`https://lchase.github.io/yak-harness/`)
  on push to `docs-site/**`. One placeholder doc; real content is #29.
