...
describe("prUrlLooksValid", () => {
  test("only a PR URL on the configured repo passes", () => {
    ...
    expect(prUrlLooksValid("https://github.com/lchase/yak/pull/12", repo)).toBe(true);
    expect(prUrlLooksValid("https://github.com/lchase/yak/pull/12 ", repo)).toBe(true);
    expect(prUrlLooksValid("https://github.com/lchase/other/pull/12", repo)).toBe(false);
    expect(prUrlLooksValid("https://github.com/lchase/yak/issues/12", repo)).toBe(false);
    expect(prUrlLooksValid("--repo=x", repo)).toBe(false);
    expect(prUrlLooksValid("https://evil.example/lchase/yak/pull/12", repo)).toBe(false);
  });
});
...
describe("parseIssueListJson", () => {
  test("flattens gh label objects to names", () => {
    const json = JSON.stringify([
      { number: 7, title: "T", labels: [{ name: "yak" }, { name: "yak:running" }] },
    ]);
  ...
  test("non-JSON raises a typed ObserveError with a readable cause", () => {
    expect(() => parseIssueListJson("gh: rate limit exceeded")).toThrow(ObserveError);
  });
});
...
describe("parseCommentsJson", () => {
  test("pulls body / authorAssociation / createdAt", () => {
    const json = JSON.stringify({
      comments: [{ body: "hi", authorAssociation: "OWNER", createdAt: "2026-01-01T00:00:00Z" }],
    });
    expect(parseCommentsJson(json)[0]).toMatchObject({ body: "hi", authorAssociation: "OWNER" });
  });
});

describe("parsePrViewJson / parsePrListJson", () => {
  test("view: well-formed → record, malformed → null", () => {
    expect(parsePrViewJson('{"state":"OPEN","mergedAt":null}')).toEqual({ state: "OPEN", mergedAt: null });
    expect(parsePrViewJson('{"state":"OPEN"}')).toBeNull();
  });
  test("list: first row or null", () => {
    expect(parsePrListJson('[{"state":"MERGED","mergedAt":"2026-01-01T00:00:00Z"}]')).toEqual({
      state: "MERGED",
...
describe("parsePendingJson", () => {
  ...
  test("normalises id/runId and steps/pending aliases", () => {
    const json = JSON.stringify({
      runs: [
        { id: "r1", pending: [{ id: "confirm-scope", kind: "gate", rendered: "Proceed?" }] },
      ],
    });
    expect(parsePendingJson(json, warn)).toEqual([
      { runId: "r1", steps: [{ stepId: "confirm-scope", kind: "gate", rendered: "Proceed?" }] },
    ]);
  });
